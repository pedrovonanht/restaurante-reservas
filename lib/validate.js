...
export function validateRestaurant({ name, max_covers }) {
  const errors = {};
  if (!name || name.trim().length < 2)
    errors.name = "Use ao menos 2 caracteres.";
  const n = Number(max_covers);
  if (!Number.isInteger(n) || n <= 0)
    errors.max_covers = "Informe uma capacidade válida.";
  return errors;
}

export function validateEvent({ name, capacity, event_date, event_times }) {
  const errors = {};
  ...
  if (capacity !== "" && capacity != null) {
    ...
    if (!Number.isInteger(n) || n <= 0)
      errors.capacity = "Capacidade deve ser um inteiro positivo.";
  }
...
export function validatePreset({ name, capacity, event_times }) {
  const errors = {};
  ...
  const n = Number(capacity);
  if (capacity === "" || capacity == null || !Number.isInteger(n) || n <= 0)
